            ...
            if temp_id:
                ...
                for order in self:
                    ...
                    for seller in seller_objs:
                        template_obj.with_context(seller=seller,company=self.env.company).send_mail(order.id, True)
        return res
    ...
    for line in self.order_line:
        if line.product_id.type == 'product' and line.product_id.inventory_availability in ['always', 'threshold']:
            ...
            if cart_qty > available_qty and (line_id == line.id):
                ...
                new_val = super(WebsiteSaleStock, self)._cart_update(line.product_id.id, line.id, qty, 0, **kwargs)
