...
class ResPartner(models.Model):
    ...
    commission = fields.Float(string="Default Sale Commission", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_commission'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    ...
    seller_payment_limit = fields.Integer(string="Seller Payment Limit", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_seller_payment_limit'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    next_payment_request = fields.Integer(string="Next Payment Request", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_next_payment_request'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    ...
    def notify_via_mail_to_seller(self, mail_templ_id):
        ...
        template_obj.with_context(company=self.env.company).send_mail(self.id, True)
